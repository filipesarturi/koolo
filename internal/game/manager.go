...
// CloseWindow sends WM_CLOSE to the game window, allowing the game to save before closing.
// This is a graceful exit method that gives the game a chance to save state.
func (gm *Manager) CloseWindow() error {
	win.PostMessage(gm.gr.HWND, win.WM_CLOSE, 0, 0)

	// Wait for the game to close with timeout
	...
	for {
		select {
		case <-timeout:
			return errors.New("timeout waiting for game to close after WM_CLOSE")
		case <-ticker.C:
			if !gm.gr.InGame() {
				return nil
			}
...
// GetHWND returns the game window handle
func (gm *Manager) GetHWND() win.HWND {
	return gm.gr.HWND
}
...
func isGPUErrorWindow(hwnd windows.HWND) bool {
	...
	enumChildWindows := windows.NewLazySystemDLL("user32.dll").NewProc("EnumChildWindows")
	enumChildWindows.Call(uintptr(hwnd), cb, 0)
